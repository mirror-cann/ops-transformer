...
class KvRmsNormRopeCacheB16MTPQUANT :
public KernelKvRmsNormRopeCacheMTPQuant<isPagedAttention, KV_DTYPE, K_DTYPE, V_DTYPE>
{
public:
    ...
    __aicore__ inline void Init(
        ...
        GM_ADDR k_rope_scale, GM_ADDR c_kv_scale, GM_ADDR k_rope_offset, GM_ADDR c_kv_offset, GM_ADDR v)
    {
        ...
        if (methodMode == 1) {
            ...
            this->kvGm.SetGlobalBuffer(
                (__gm__ KV_DTYPE*)kv +
                GetBlockIdx() * this->tilingData_->rowsPerBlock *
                this->tilingData_->numHead * this->tilingData_->seqLength * this->RMS_NORM_LENGTH);
            this->vGm.SetGlobalBuffer(
                (__gm__ KV_DTYPE*)v +
                GetBlockIdx() * this->tilingData_->rowsPerBlock *
                this->tilingData_->numHead * this->tilingData_->seqLength * this->V_LENGTH);
            this->gammaGm.SetGlobalBuffer((__gm__ KV_DTYPE*)gamma);
            this->cosGm.SetGlobalBuffer((__gm__ KV_DTYPE*)cos +
            GetBlockIdx() * this->tilingData_->rowsPerBlock * this->tilingData_->numHead * this->ROPE_LENGTH);
            ...
            if constexpr (isPagedAttention) {
                ...
                this->indexGm.SetGlobalBuffer(
                    (__gm__ int64_t*)index +
                    GetBlockIdx() * this->tilingData_->rowsPerBlock * this->tilingData_->seqLength);
                this->kCacheGm.SetGlobalBuffer(
                    (__gm__ K_DTYPE*)k_cache +
                    GetBlockIdx() * this->tilingData_->rowsPerBlock *
                    this->tilingData_->numHead * this->tilingData_->cacheLength * this->RMS_NORM_LENGTH);
                this->vCacheGm.SetGlobalBuffer(
                    (__gm__ V_DTYPE*)v_cache +
                    GetBlockIdx() * this->tilingData_->rowsPerBlock *
                    this->tilingData_->numHead * this->tilingData_->cacheLength * this->V_LENGTH);
            }
            ...
            this->pipe_->InitBuffer(this->inQueueX, 1,
                this->tilingData_->rowsPerBlock * this->RMS_NORM_LENGTH * sizeof(float));
            this->pipe_->InitBuffer(this->inQueueV, 1,
                this->tilingData_->rowsPerBlock * this->V_LENGTH * sizeof(float));
            ...
            this->pipe_->InitBuffer(this->inQueueScaleOffset, 1,
                (2 * (this->RMS_NORM_LENGTH + this->V_LENGTH)) * sizeof(float));
            this->pipe_->InitBuffer(this->bufferXFp32,
                this->tilingData_->rowsPerBlock * this->RMS_NORM_LENGTH * sizeof(float));
            this->pipe_->InitBuffer(this->bufferXSquare,
                2 * this->tilingData_->rowsPerBlock * this->RMS_NORM_LENGTH * sizeof(float));
            this->pipe_->InitBuffer(this->bufferSum,
                ((this->tilingData_->rowsPerBlock + NUM_EIGHT - 1) / NUM_EIGHT) * NUM_EIGHT * sizeof(float));
            this->pipe_->InitBuffer(this->outQueueV, 1,
                2 * this->tilingData_->rowsPerBlock * this->V_LENGTH * sizeof(KV_DTYPE));
            this->pipe_->InitBuffer(this->outQueueK, 1,
                2 * this->tilingData_->rowsPerBlock * this->RMS_NORM_LENGTH * sizeof(KV_DTYPE));
            ...
            for (int64_t n = 0; n < this->tilingData_->numHead; n++) {
                for (int64_t i = 0; i < this->tilingData_->seqLength; i++) {
                    ...
                    if (this->tilingData_->isKQuant) {
                        ...
                        if (isKOffset == 1) {
                            DataCopy(kOffsetLocal,
                                this->kRopeOffsetGm[n * this->RMS_NORM_LENGTH], this->RMS_NORM_LENGTH);
                        }
                    }
                    if (this->tilingData_->isVQuant) {
                        DataCopy(vScaleLocal, this->cKvScaleGm[n * this->V_LENGTH], this->V_LENGTH);
                        if (isVOffset == 1) {
                            DataCopy(vOffsetLocal,
                                this->cKvOffsetGm[n * this->V_LENGTH], this->V_LENGTH);
                            }
                    }
                    ...
                    static_cast<uint32_t>(this->RMS_NORM_LENGTH * sizeof(KV_DTYPE)),
                    static_cast<uint32_t>(
                        ((this->tilingData_->seqLength - 1) * this->RMS_NORM_LENGTH +
                        (this->tilingData_->numHead - 1) *
                        this->tilingData_->seqLength * this->RMS_NORM_LENGTH) * sizeof(KV_DTYPE)),
                    0, 0};
                    DataCopyPadExtParams<KV_DTYPE> padXParams{false, 0, 0, 0};
                    int64_t nXOffset = n * this->tilingData_->seqLength * this->RMS_NORM_LENGTH +
                    i * this->RMS_NORM_LENGTH;
                    ...
                    Cast(xFp32Local, xB16Local, RoundMode::CAST_NONE,
                    ...
                    LocalTensor<KV_DTYPE> sinLocal =
                    cosLocal[this->tilingData_->rowsPerBlock * this->ROPE_LENGTH * NUM_TWO];
                    LocalTensor<KV_DTYPE> cosLocalB16 =
                    cosLocal[this->tilingData_->rowsPerBlock * this->ROPE_LENGTH];
                    LocalTensor<KV_DTYPE> sinLocalB16 =
                    sinLocal[this->tilingData_->rowsPerBlock * this->ROPE_LENGTH];
                    ...
                    DataCopyExtParams copyCosSinParams{
                        ...
                        static_cast<uint32_t>((this->tilingData_->numHead - 1)
                        * this->ROPE_LENGTH * sizeof(KV_DTYPE)), 0, 0};
                    ...
                    LocalTensor<int8_t> outLocalKQuant =
                    outLocalK.template ReinterpretCast<int8_t>()[
                        this->tilingData_->rowsPerBlock * this->RMS_NORM_LENGTH * sizeof(KV_DTYPE)];
                    ...
                    RopeKV2<KV_DTYPE>(xLocal, xFp32Local, cosLocal, sinLocal, cosLocalB16, sinLocalB16,
                        outLocalKQuant, kScaleLocal, kOffsetLocal, isKOffset, this->RMS_NORM_LENGTH);
                    ...
                    if (this->tilingData_->isKQuant) {
                        ScatterUpdate<int8_t, KV_DTYPE>(
                            (GlobalTensor<int8_t>&)this->kCacheGm, this->kCacheGmNd,
                            outLocalK, outLocalKQuant, n, i, this->RMS_NORM_LENGTH);
                    } else {
                    ...
                    static_cast<uint16_t>(this->tilingData_->rowsPerBlock),
                    static_cast<uint32_t>(this->V_LENGTH * sizeof(KV_DTYPE)),
                    static_cast<uint32_t>(
                        ((this->tilingData_->seqLength - 1) * this->V_LENGTH +
                        (this->tilingData_->numHead - 1) *
                        this->tilingData_->seqLength * this->V_LENGTH) * sizeof(KV_DTYPE)),
                    0, 0};
                    ...
                    LocalTensor<int8_t> outLocalVQuant =
                    outLocalV.template ReinterpretCast<int8_t>()[
                        this->tilingData_->rowsPerBlock * this->V_LENGTH * sizeof(KV_DTYPE)];
                    DataCopy(outLocalV, yLocal, this->tilingData_->rowsPerBlock * this->V_LENGTH);
                    SetWaitFlag<HardEvent::V_V>(HardEvent::V_V);

                    if (this->tilingData_->isVQuant) {
                        Cast(workspaceBuffer, yLocal, RoundMode::CAST_NONE,
                            this->tilingData_->rowsPerBlock * this->V_LENGTH);
                        PipeBarrier<PIPE_V>();
                        Quant<KV_DTYPE>(outLocalVQuant, workspaceBuffer,
                            vScaleLocal, vOffsetLocal, isVOffset, this->V_LENGTH);
                    }
                    ...
                    if (this->tilingData_->isVQuant) {
                        ScatterUpdate<int8_t, KV_DTYPE>(
                            (GlobalTensor<int8_t>&)this->vCacheGm,
                            this->vCacheGmNd, outLocalV, outLocalVQuant, n, i, this->V_LENGTH);
                    } else {
    ...
    __aicore__ inline void RopeKV2(
        const LocalTensor<T>& xLocal, const LocalTensor<float>& xFp32Local,
        const LocalTensor<T>& cosLocal, const LocalTensor<T>& sinLocal,
        const LocalTensor<T>& cosLocalB16, const LocalTensor<T>& sinLocalB16,
        const LocalTensor<int8_t>& outLocalKQuant, const LocalTensor<float>& scaleLocal,
        const LocalTensor<float>& offsetLocal, bool ifAsymmetricQuant, int64_t kvLength)
    {
        ...
        LocalTensor<KV_DTYPE> imagLocal = realLocal[
            this->tilingData_->rowsPerBlock * this->ROPE_LENGTH_HALF * NUM_TWO];  // 32 * sizeof(float)
        LocalTensor<KV_DTYPE> buf_ = imagLocal[
            this->tilingData_->rowsPerBlock * this->ROPE_LENGTH_HALF * NUM_TWO];  // 64 * sizeof(float)
        ...
        uint64_t rsvdCnt = 0;
        GatherMask(
            realLocalFp32, xFp32Local, 1, true, this->ROPE_LENGTH,
            {1, static_cast<uint16_t>(this->tilingData_->rowsPerBlock), 2 * NUM_TWELVE, 0}, rsvdCnt);
        GatherMask(
            imagLocalFp32, xFp32Local, NUM_TWO, true, this->ROPE_LENGTH,
            {1, static_cast<uint16_t>(this->tilingData_->rowsPerBlock), 2 * NUM_TWELVE, 0},
            rsvdCnt);
        PipeBarrier<PIPE_V>();
        ...
        Mul(bufFp32, imagLocalFp32, sinLocalFp32, mask, this->tilingData_->rowsPerBlock,
            {1, 1, 1, NUM_EIGHT, NUM_FOUR, NUM_EIGHT});
        Mul(bufFp32[this->ROPE_LENGTH_HALF], realLocalFp32,
            sinLocalFp32[this->ROPE_LENGTH_HALF], mask, this->tilingData_->rowsPerBlock,
            {1, 1, 1, NUM_EIGHT, NUM_FOUR, NUM_EIGHT});
        PipeBarrier<PIPE_V>();
        ...
        if constexpr (std::is_same<KV_DTYPE, bfloat16_t>::value) {
            Cast(xLocal, outLocalFp32, RoundMode::CAST_RINT,
                this->ROPE_LENGTH, this->tilingData_->rowsPerBlock, {1, 1, NUM_TWELVE, NUM_EIGHT});
        } else if constexpr (std::is_same<KV_DTYPE, half>::value) {
            Cast(xLocal, outLocalFp32, RoundMode::CAST_NONE,
                this->ROPE_LENGTH, this->tilingData_->rowsPerBlock, {1, 1, NUM_TWELVE, NUM_EIGHT});
        }

        if (this->tilingData_->isKQuant) {
            PipeBarrier<PIPE_V>();
            Quant<KV_DTYPE>(outLocalKQuant, xFp32Local,
                scaleLocal, offsetLocal, ifAsymmetricQuant, this->RMS_NORM_LENGTH);
        }
    }
    ...
    __aicore__ inline void RmsNormV2(const LocalTensor<float>& xFp32Local, const LocalTensor<T>& gammaLocal)
    {
        ...
        Add(xSquareLocal[0], xSquareLocal[0],
            xSquareLocal[NUM_FORTY_EIGHT], NUM_FORTY_EIGHT, NUM_TWO * this->tilingData_->rowsPerBlock,
            {1, 1, 1, NUM_SIX, NUM_TWELVE, NUM_TWELVE}); // [this->tilingData_->rowsPerBlock, 96]
        PipeBarrier<PIPE_V>();
        Add(xSquareLocal[0], xSquareLocal[0],
            xSquareLocal[NUM_FORTY_EIGHT], NUM_FORTY_EIGHT, this->tilingData_->rowsPerBlock,
        ...
        for (int64_t rowId = 0; rowId < this->tilingData_->rowsPerBlock; rowId++) {
            Div(xFp32Local[rowId * this->RMS_NORM_LENGTH], xFp32Local[rowId * this->RMS_NORM_LENGTH],
                xSquareLocal[rowId * NUM_EIGHT], mask,
                (this->RMS_NORM_LENGTH / NUM_SIXTY_FOUR), {1, 1, 0, NUM_EIGHT, NUM_EIGHT, 0});
        }
        PipeBarrier<PIPE_V>();
        ...
        const LocalTensor<T1>& quantLocal, int64_t nIdx, int64_t seqId, int64_t kvLength)
    {
        ...
        if (this->tilingData_->isOutputKv) {
            int64_t gmOffsetNd = GetBlockIdx() * this->tilingData_->rowsPerBlock *
                                    this->tilingData_->numHead * this->tilingData_->seqLength * kvLength +
    ...
    __aicore__ inline void Quant(
        const LocalTensor<int8_t>& quantOutLocal, const LocalTensor<float>& srcTensor,
        const LocalTensor<float>& scaleLocal, const LocalTensor<float>& offsetLocal,
        bool ifAsymmetricQuant, int64_t kvLength)
    {
        int64_t totalMask = kvLength;
        while (totalMask > 0) {
            int64_t offset = kvLength - totalMask;
            Mul(srcTensor[offset], srcTensor[offset], scaleLocal[offset],
                totalMask >= NUM_SIXTY_FOUR ? NUM_SIXTY_FOUR : totalMask,   // mask
                this->tilingData_->rowsPerBlock, {1, 1, 1,  // repeat
                static_cast<uint8_t>(kvLength / NUM_EIGHT), static_cast<uint8_t>(kvLength / NUM_EIGHT), 0});
