...
__simd_vf__ void ProcessVec1UpdateGeneralImpl256Mxfp8FullquantVFSubloop0(
    ...
    __ubuf__ fp8_e8m0_t *pScaleSubLoop0, __ubuf__ float *preLoopMaxUb, __ubuf__ float *preLoopSumUb,
    ...
    const float dScaleQK, const T minValue, const float sinkValue = 0.0f, const float pScale = 1.0f)
{
    ...
    RegTensor<float> vreg_sel0;
    RegTensor<float> vreg_sel1;
    RegTensor<float> vreg_sel2;
    RegTensor<float> vreg_sel3;
    RegTensor<float> vreg_src_x1;
    RegTensor<float> vreg_src_x2;
    RegTensor<float> vreg_src_x3;
    RegTensor<float> vreg_src_x4;
    RegTensor<float> vreg_sel2_new;
    RegTensor<float> vreg_sel3_new;
    RegTensor<float> vreg_src_x3_new;
    RegTensor<float> vreg_src_x4_new;
    RegTensor<float> vreg_max_tmp0;
    ...
    RegTensor<float> vreg_exp_even0;
    RegTensor<float> vreg_exp_odd0;
    ...
    RegTensor<bfloat16_t> vreg_exp_even0_bf16;
    RegTensor<bfloat16_t> vreg_exp_odd0_bf16;
    RegTensor<bfloat16_t> vreg_exp_even1_bf16;
    RegTensor<bfloat16_t> vreg_exp_odd1_bf16;
    RegTensor<bfloat16_t> vreg_pse0_bf16;
    ...
    RegTensor<half> vreg_exp_even0_f16;
    RegTensor<half> vreg_exp_odd0_f16;
    RegTensor<half> vreg_exp_even1_f16;
    RegTensor<half> vreg_exp_odd1_f16;
    RegTensor<half> vreg_pse0_f16;
    ...
    // mxfp8
    RegTensor<fp8_e8m0_t> vreg_p_scale_fp8;
    ...
    MaskReg preg_tail_n1 = UpdateMask<T>(pltTailN1); // tail
    ...
    if constexpr (pseMode == PseTypeEnum::PSE_INNER_MUL_ADD_TYPE ||
        ...
        Arange(vreg_alibi0, posShift);
        Arange(vreg_alibi1, posShift + floatRepSize);
        Arange(vreg_alibi2, posShift + floatRepSize * 2);
        Arange(vreg_alibi3, posShift + floatRepSize * 3);
    }
    for (uint16_t i = 0; i < m; ++i) {
        LoadAlign(vreg_src_x1, srcUb + i * s2BaseSize);
        LoadAlign(vreg_src_x2, srcUb + floatRepSize + i * s2BaseSize);
        LoadAlign(vreg_src_x3, srcUb + floatRepSize * 2 + i * s2BaseSize);
        LoadAlign(vreg_src_x4, srcUb + floatRepSize * 3 + i * s2BaseSize);
        if constexpr (pseMode != PseTypeEnum::PSE_OUTER_ADD_MUL_TYPE) {
            // Muls(scale)
            Muls(vreg_src_x1, vreg_src_x1, scale, preg_all);
            Muls(vreg_src_x2, vreg_src_x2, scale, preg_all);
            Muls(vreg_src_x3, vreg_src_x3, scale, preg_ori_tail_n1);
            Muls(vreg_src_x4, vreg_src_x4, scale, preg_ori_tail_n2);
        }
        if constexpr (pseMode != PseTypeEnum::PSE_NONE_TYPE) {
            if constexpr (pseMode == PseTypeEnum::PSE_INNER_MUL_ADD_TYPE ||
                          pseMode == PseTypeEnum::PSE_INNER_MUL_ADD_SQRT_TYPE) { // alibi
                Abs(vreg_pse0, vreg_alibi0, preg_all);
                ...
                if constexpr (IsSameType<pseShiftType, bfloat16_t>::value) {
                    ...
                    Interleave(vreg_pse0_bf16, vreg_pse1_bf16, vreg_pse_bf16_src1, vreg_pse_bf16_src1);
                    Interleave(vreg_pse2_bf16, vreg_pse3_bf16, vreg_pse_bf16_src2, vreg_pse_bf16_src2);
                    Cast<T, pseShiftType, castTraitZero>(vreg_pse0, vreg_pse0_bf16, preg_all_b16);
                    ...
                    Interleave(vreg_pse0_f16, vreg_pse1_f16, vreg_pse_f16_src1, vreg_pse_f16_src1);
                    Interleave(vreg_pse2_f16, vreg_pse3_f16, vreg_pse_f16_src2, vreg_pse_f16_src2);
                    // Cast bfloat16 to target type T
                    Cast<T, pseShiftType, castTraitZero>(vreg_pse0, vreg_pse0_f16, preg_all_b16);
            ...
            // Add PSE bias
            Add(vreg_src_x1, vreg_src_x1, vreg_pse0, preg_all);
            Add(vreg_src_x2, vreg_src_x2, vreg_pse1, preg_all);
            Add(vreg_src_x3, vreg_src_x3, vreg_pse2, preg_ori_tail_n1);
            Add(vreg_src_x4, vreg_src_x4, vreg_pse3, preg_ori_tail_n2);
        }
        if constexpr (pseMode == PseTypeEnum::PSE_OUTER_ADD_MUL_TYPE) {
            // Muls(scale)
            Muls(vreg_src_x1, vreg_src_x1, scale, preg_all);
        ...
        if constexpr (hasAtten == 1) {
            ...
            Select(vreg_sel0, vreg_min, vreg_src_x1, preg_compare1);
            Select(vreg_sel1, vreg_min, vreg_src_x2, preg_compare2);
            Select(vreg_sel2, vreg_min, vreg_src_x3, preg_compare3);
            Select(vreg_sel3, vreg_min, vreg_src_x4, preg_compare4);
            Select(vreg_sel2_new, vreg_sel2, vreg_min, preg_ori_tail_n1);
            Select(vreg_sel3_new, vreg_sel3, vreg_min, preg_ori_tail_n2);
            // Store masked results back to UB
            StoreAlign<T, MicroAPI::StoreDist::DIST_NORM_B32>((__ubuf__ T *&)srcUb + i * s2BaseSize, vreg_sel0,
                                                              preg_all);
            ...
            StoreAlign<T, MicroAPI::StoreDist::DIST_NORM_B32>((__ubuf__ T *&)srcUb + floatRepSize * 2 + i * s2BaseSize,
                                                              vreg_sel2_new, preg_all);
            StoreAlign<T, MicroAPI::StoreDist::DIST_NORM_B32>((__ubuf__ T *&)srcUb + floatRepSize * 3 + i * s2BaseSize,
                                                              vreg_sel3_new, preg_all);
            Max(vreg_max_tmp0, vreg_sel0, vreg_sel1, preg_all);
            Max(vreg_max_tmp1, vreg_sel2_new, vreg_sel3_new, preg_all);
            Max(vreg_max_tmp2, vreg_max_tmp0, vreg_max_tmp1, preg_all);
            Reduce<MicroAPI::ReduceType::MAX, float, float, MicroAPI::MaskMergeMode::ZEROING>(vreg_src_max,
                                                                                              vreg_max_tmp2, preg_all);
        } else {
            Select(vreg_src_x3_new, vreg_src_x3, vreg_min, preg_ori_tail_n1);
            Select(vreg_src_x4_new, vreg_src_x4, vreg_min, preg_ori_tail_n2); // select tail
            StoreAlign<T, MicroAPI::StoreDist::DIST_NORM_B32>((__ubuf__ T *&)srcUb + i * s2BaseSize, vreg_src_x1,
                                                              preg_all);
            StoreAlign<T, MicroAPI::StoreDist::DIST_NORM_B32>((__ubuf__ T *&)srcUb + floatRepSize + i * s2BaseSize,
                                                              vreg_src_x2, preg_all);
            StoreAlign<T, MicroAPI::StoreDist::DIST_NORM_B32>((__ubuf__ T *&)srcUb + floatRepSize * 2 + i * s2BaseSize,
                                                              vreg_src_x3_new, preg_all);
            StoreAlign<T, MicroAPI::StoreDist::DIST_NORM_B32>((__ubuf__ T *&)srcUb + floatRepSize * 3 + i * s2BaseSize,
                                                              vreg_src_x4_new, preg_all);
            Max(vreg_max_tmp0, vreg_src_x1, vreg_src_x2, preg_all);
            Max(vreg_max_tmp1, vreg_src_x3_new, vreg_src_x4_new, preg_all);
            Max(vreg_max_tmp2, vreg_max_tmp0, vreg_max_tmp1, preg_all);
            Reduce<MicroAPI::ReduceType::MAX, float, float, MicroAPI::MaskMergeMode::ZEROING>(vreg_src_max,
                                                                                              vreg_max_tmp2, preg_all);
        }
    ...
    LoadAlign(vreg_src_max, tmpMaxUb2);
    StoreAlign<T, MicroAPI::StoreDist::DIST_NORM_B32>((__ubuf__ T *&)preLoopMaxUb, vreg_in_max, preg_all);
    Max(vreg_max_new, vreg_in_max, vreg_src_max, preg_all);
    ...
    for (uint16_t i = 0; i < m; ++i) {
        ...
        LoadAlign<T, MicroAPI::LoadDist::DIST_DINTLV_B32>(vreg_src_x1, vreg_src_x2, srcUb + i * s2BaseSize);
        LoadAlign<T, MicroAPI::LoadDist::DIST_DINTLV_B32>(vreg_src_x3, vreg_src_x4,
                                                          srcUb + floatRepSize * 2 + i * s2BaseSize);
        ExpSub(vreg_exp_even0, vreg_src_x1, vreg_max, preg_all);
        ExpSub(vreg_exp_odd0, vreg_src_x2, vreg_max, preg_all);
        ExpSub(vreg_exp_even1, vreg_src_x3, vreg_max, preg_all);
        ExpSub(vreg_exp_odd1, vreg_src_x4, vreg_max, preg_all);

        Add(vreg_exp_sum1, vreg_exp_even0, vreg_exp_odd0, preg_all); // x_sum = sum(x_exp, axis=-1, keepdims=True)
        Add(vreg_exp_sum2, vreg_exp_even1, vreg_exp_odd1, preg_all);
        Add(vreg_exp_sum3, vreg_exp_sum1, vreg_exp_sum2, preg_all);
        Reduce<MicroAPI::ReduceType::SUM, float, float, MicroAPI::MaskMergeMode::ZEROING>(vreg_exp_sum3,
            vreg_exp_sum3, preg_all);
        StoreUnAlign<float, MicroAPI::PostLiteral::POST_MODE_UPDATE>(((__ubuf__ T *&)tmpExpSumUb),
            vreg_exp_sum3, ureg_exp_sum, 1);
        // dropmask compute
        if constexpr (hasDrop == 1) {
            ...
            Select(vreg_sel_drop, vreg_exp_even0, vreg_zero, preg5);
            Muls(vreg_exp_even0, vreg_sel_drop, divValue, preg_all);
            Select(vreg_sel_drop_2, vreg_exp_odd0, vreg_zero, preg6);
            Muls(vreg_exp_odd0, vreg_sel_drop_2, divValue, preg_all);
            LoadAlign<uint32_t, MicroAPI::PostLiteral::POST_MODE_UPDATE, MicroAPI::MaskDist::DIST_US>(
            ...
            Select(vreg_sel_drop_2, vreg_exp_odd1, vreg_zero, preg6);
            Muls(vreg_exp_odd1, vreg_sel_drop_2, divValue, preg_all);
        }

        if constexpr (IsSameType<T2, bfloat16_t>::value) {
            Cast<T2, T, castTraitZero>(vreg_exp_even0_bf16, vreg_exp_even0, preg_all);
            Cast<T2, T, castTraitOne>(vreg_exp_odd0_bf16, vreg_exp_odd0, preg_all);
            ...
            Or((RegTensor<uint16_t> &)vreg_exp1_bf16, (RegTensor<uint16_t> &)vreg_exp_even0_bf16,
               (RegTensor<uint16_t> &)vreg_exp_odd0_bf16, preg_all_b16);
            Or((RegTensor<uint16_t> &)vreg_exp2_bf16, (RegTensor<uint16_t> &)vreg_exp_even1_bf16,
            ...
            Cast<T2, T, castTraitRintZero>(vreg_exp_even_f8e4m3_1, vreg_exp_even0, preg_all);
            Cast<T2, T, castTraitRintZero>(vreg_exp_even_f8e4m3_2, vreg_exp_even1, preg_all);
            Cast<T2, T, castTraitRintTwo>(vreg_exp_odd_f8e4m3_1, vreg_exp_odd0, preg_all);
            Cast<T2, T, castTraitRintTwo>(vreg_exp_odd_f8e4m3_2, vreg_exp_odd1, preg_all);
            ...
            // cast to fp16
            Cast<T2, T, castTraitZero>(vreg_exp_even0_f16, vreg_exp_even0, preg_all);
            Cast<T2, T, castTraitOne>(vreg_exp_odd0_f16, vreg_exp_odd0, preg_all);
            ...
            Or((RegTensor<uint16_t> &)vreg_exp1_f16, (RegTensor<uint16_t> &)vreg_exp_even0_f16,
               (RegTensor<uint16_t> &)vreg_exp_odd0_f16, preg_all_b16);
            Or((RegTensor<uint16_t> &)vreg_exp2_f16, (RegTensor<uint16_t> &)vreg_exp_even1_f16,
    ...
    // store sum
    StoreUnAlignPost<float, MicroAPI::PostLiteral::POST_MODE_UPDATE>(((__ubuf__ T *&)tmpExpSumUb),
                                                                     ureg_exp_sum, 0);
    LocalMemBar<MemType::VEC_STORE, MemType::VEC_LOAD>();
    ...
    StoreAlign<T, MicroAPI::StoreDist::DIST_NORM_B32>((__ubuf__ T *&)firstLoopSumUb,
        vreg_exp_sum_brc, preg_all); // 传给loop 1用于更新rowSum
    Duplicate(vreg_p_scale_fp8, 0x7f, preg_all_b8);
    StoreAlign<fp8_e8m0_t, MicroAPI::StoreDist::DIST_NORM_B8>(((__ubuf__ fp8_e8m0_t *&)pScaleSubLoop0),
                                                              vreg_p_scale_fp8, preg_all_b8);
}
    ...
    const float dScaleQK, const T minValue, const float sinkValue = 0.0f, const float pScale = 1.0f)
{
    ...
    RegTensor<fp8_e8m0_t> vreg_p_scale_fp8_0;
    RegTensor<fp8_e8m0_t> vreg_p_scale_fp8_1;
    RegTensor<uint8_t> vreg_exp_merge_f8e4m3_indexes;
    RegTensor<fp8_e8m0_t> vreg_p_scale_fp8_pad;
    ...
    for (uint16_t i = 0; i < m; ++i) {
        ...
        LoadAlign(vreg_src_x4, srcUb + floatRepSize * 3 + i * s2BaseSize);
        if constexpr (pseMode != PseTypeEnum::PSE_OUTER_ADD_MUL_TYPE) {
            Muls(vreg_src_x1, vreg_src_x1, scale, preg_all); // Muls (scale)
            Muls(vreg_src_x2, vreg_src_x2, scale, preg_all);
            Muls(vreg_src_x3, vreg_src_x3, scale, preg_ori_tail_n1);
            Muls(vreg_src_x4, vreg_src_x4, scale, preg_ori_tail_n2);
        }
        if constexpr (pseMode != PseTypeEnum::PSE_NONE_TYPE) {
            if constexpr (pseMode == PseTypeEnum::PSE_INNER_MUL_ADD_TYPE ||
                ...
                Muls(vreg_pse0, vreg_pse0, slopes, preg_all);
                Muls(vreg_pse1, vreg_pse1, slopes, preg_all);
                Muls(vreg_pse2, vreg_pse2, slopes, preg_all);
                Muls(vreg_pse3, vreg_pse3, slopes, preg_all);
                Adds(vreg_alibi0, vreg_alibi0, -1.0f, preg_all);
        ...
        if constexpr (hasAtten == 1) {
            ...
            Select(vreg_src_x3_new, vreg_src_x3, vreg_min, preg_ori_tail_n1);
            Select(vreg_src_x4_new, vreg_src_x4, vreg_min, preg_ori_tail_n2);
            StoreAlign<T, MicroAPI::StoreDist::DIST_NORM_B32>((__ubuf__ T *&)srcUb + i * s2BaseSize, vreg_src_x1,
                                                              preg_all);
            StoreAlign<T, MicroAPI::StoreDist::DIST_NORM_B32>((__ubuf__ T *&)srcUb + floatRepSize + i * s2BaseSize,
                                                              vreg_src_x2, preg_all);
    ...
    for (uint16_t i = 0; i < m; ++i) {
        ...
        Sub(vreg_max, vreg_max, vreg_ln_p_scale, preg_all);
        LoadAlign<T, MicroAPI::LoadDist::DIST_DINTLV_B32>(vreg_src_x1, vreg_src_x2, srcUb + i * s2BaseSize);
        LoadAlign<T, MicroAPI::LoadDist::DIST_DINTLV_B32>(vreg_src_x3,
            vreg_src_x4, srcUb + floatRepSize * 2 + i * s2BaseSize);
        ExpSub(vreg_exp_even0, vreg_src_x1, vreg_max, preg_all);
        ExpSub(vreg_exp_odd0, vreg_src_x2, vreg_max, preg_all);
        ExpSub(vreg_exp_even1, vreg_src_x3, vreg_max, preg_all);
        ExpSub(vreg_exp_odd1, vreg_src_x4, vreg_max, preg_all);

        Add(vreg_exp_sum1, vreg_exp_even0, vreg_exp_odd0, preg_all);
        Add(vreg_exp_sum2, vreg_exp_even1, vreg_exp_odd1, preg_all);
        Add(vreg_exp_sum3, vreg_exp_sum1, vreg_exp_sum2, preg_all);
        Reduce<MicroAPI::ReduceType::SUM, float, float, MicroAPI::MaskMergeMode::ZEROING>(vreg_exp_sum3,
            vreg_exp_sum3, preg_all);
        ...
        if constexpr (IsSameType<T2, bfloat16_t>::value) {
            ...
            RegTensor<fp8_e4m3fn_t> vreg_exp_odd_f8e4m3_1;
            RegTensor<fp8_e4m3fn_t> vreg_exp_odd_f8e4m3_2;
            RegTensor<fp8_e4m3fn_t> vreg_exp_merge_f8e4m3_tmp_1;
            RegTensor<fp8_e4m3fn_t> vreg_exp_merge_f8e4m3_tmp_2;
            RegTensor<fp8_e4m3fn_t> vreg_exp_merge_f8e4m3_1;
            RegTensor<fp8_e4m3fn_t> vreg_exp_merge_f8e4m3_2;
            ...
            Cast<T2, T, castTraitRintTwo>(vreg_exp_odd_f8e4m3_1, vreg_exp_odd0, preg_all);
            Cast<T2, T, castTraitRintTwo>(vreg_exp_odd_f8e4m3_2, vreg_exp_odd1, preg_all);
            Or((RegTensor<uint8_t> &)vreg_exp_merge_f8e4m3_tmp_1, (RegTensor<uint8_t> &)vreg_exp_even_f8e4m3_1,
               (RegTensor<uint8_t> &)vreg_exp_odd_f8e4m3_1, preg_all_b8);
            Or((RegTensor<uint8_t> &)vreg_exp_merge_f8e4m3_tmp_2, (RegTensor<uint8_t> &)vreg_exp_even_f8e4m3_2,
               (RegTensor<uint8_t> &)vreg_exp_odd_f8e4m3_2, preg_all_b8);
            LoadAlign(vreg_exp_merge_f8e4m3_indexes, indexesUb);
            Gather(vreg_exp_merge_f8e4m3_1, vreg_exp_merge_f8e4m3_tmp_1, vreg_exp_merge_f8e4m3_indexes);
            Gather(vreg_exp_merge_f8e4m3_2, vreg_exp_merge_f8e4m3_tmp_2, vreg_exp_merge_f8e4m3_indexes);
            StoreAlign<T2, MicroAPI::DataCopyMode::DATA_BLOCK_COPY, MicroAPI::PostLiteral::POST_MODE_UPDATE>(
                ((__ubuf__ T2 *&)expUb1), vreg_exp_merge_f8e4m3_1, blockStride, repeatStride, preg_all_b8_128);
            StoreAlign<T2, MicroAPI::DataCopyMode::DATA_BLOCK_COPY, MicroAPI::PostLiteral::POST_MODE_UPDATE>(
                ((__ubuf__ T2 *&)expUb2), vreg_exp_merge_f8e4m3_2, blockStride, repeatStride, preg_all_b8_128);
    ...
    // pScale
    RegTensor<fp8_e8m0_t> vreg_p_scale_fp8_dst0;
    RegTensor<fp8_e8m0_t> vreg_p_scale_fp8_dst1;
    ...
    Cast<fp8_e8m0_t, bfloat16_t, castTraitNoneZero>(vreg_p_scale_fp8_0, vreg_p_scale_bf16_0, preg_all_b16);
    Cast<bfloat16_t, T, castTraitRintOne>(vreg_p_scale_bf16_1, vreg_p_scale, preg_all);
    Cast<fp8_e8m0_t, bfloat16_t, castTraitNoneZero>(vreg_p_scale_fp8_1, vreg_p_scale_bf16_1, preg_all_b16);
    Or((RegTensor<uint8_t> &)vreg_p_scale_fp8_0, (RegTensor<uint8_t> &)vreg_p_scale_fp8_0,
        (RegTensor<uint8_t> &)vreg_p_scale_fp8_1, preg_all_b8);
    Duplicate(vreg_p_scale_fp8_1, 0x7f, preg_all_b8);
    DeInterleave(vreg_p_scale_fp8_dst0, vreg_p_scale_fp8_dst1, vreg_p_scale_fp8_0, vreg_p_scale_fp8_1);
    StoreAlign<fp8_e8m0_t, MicroAPI::StoreDist::DIST_NORM_B8>(((__ubuf__ fp8_e8m0_t *&)pScaleSubLoop0),
        vreg_p_scale_fp8_dst0, preg_all_b8);
}
    ...
    const T minValue, float keepProb, const float sinkValue, const float pScale)
{
    ...
    if (subLoop == 0) {
        ProcessVec1UpdateGeneralImpl256Mxfp8FullquantVFSubloop0<T, T2, pseShiftType, s1BaseSize, s2BaseSize, hasAtten,
            ...
            pScaleSubLoop0Ub, preLoopMaxUb, preLoopSumUb, firstLoopSumUb, nPadding, blockStride, repeatStride,
            oriTailN1, oriTailN2, tailN1, tailN2, pltOriTailN1, pltOriTailN2, pltTailN1, pltTailN2, divValue, pltN, m,
            pseStride, slopes, posShift, scale, dScaleQK, minValue, sinkValue, pScale);
    } else {
