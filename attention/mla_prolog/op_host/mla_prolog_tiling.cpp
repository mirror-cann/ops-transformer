...
QUANT_MODE MlaPrologTiling::GetQuantizationModeV3() const
{
    if (*(context_->weightQuantMode) == static_cast<int>(WEIGHT_QUANT_MODE::NO_QUANT)) {
        if (*(context_->kvQuantMode) == static_cast<int>(KV_QUANT_MODE::NO_QUANT)) {
            ...
            OP_LOGE(context_->opName, "When weightQuantMode == 0, kvQuantMode must be within {0}, actually is %d.",
        ...
        OP_LOGE(context_->opName, "WeightQuantMode must be within {0, 1, 2}, actually is %d.",
...
QUANT_MODE MlaPrologTiling::GetQuantizationModeV3Mxfp8() const
{
    if (*(context_->weightQuantMode) == static_cast<int>(WEIGHT_QUANT_MODE::NO_QUANT)) {
        ...
        if (*(context_->kvQuantMode) == static_cast<int>(KV_QUANT_MODE::NO_QUANT)) {
            ...
            OP_LOGE(context_->opName,
                    "When weightQuantMode == 3, kvQuantMode must be within {0, 1, 3}, actually is %d.",
                    *(context_->kvQuantMode));
        }
    } else {
        OP_LOGE(context_->opName, "weightQuantMode must be within {0, 1, 2, 3, 4, 5}, actually is %d.",
                *(context_->weightQuantMode));
    }
    return QUANT_MODE::ERROR_MODE;
}
    ...
    if (scenarioInfo_.batchSeqFusedFlag_ &&
        ...
        OP_CHECK_IF(context_->actualSeqLen.shape == nullptr,
                    OP_LOGE(context_->opName,
                            ...
                            "actualSeqLen should not be null."),
                    return GRAPH_FAILED);
...
ge::graphStatus MlaPrologTiling::GenTilingKey() const
{
    ...
    if (cvMode == ASCENDC_TPL_MIX_AIC_1_1 &&
        ...
        OP_LOGE(context_->opName,
                "CV1:1 mode only support quantMode is in {NO_QUANT} and cacheMode is in {PA_BSND,PA_NZ}, quantMode is "
                "%d, cacheMode is %u.",
...
{
    OP_CHECK_IF(context.GetNodeName() == nullptr, OP_LOGE(V1_OP_NAME, "OpName got from TilingContext is nullptr."),
                return ge::GRAPH_FAILED);
