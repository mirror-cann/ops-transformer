...
static inline uint32_t FindBestSingleNA8W4(uint32_t baseM_, uint32_t baseN_, uint32_t avg_m, uint32_t maxN_,
                                           uint32_t groupNum_, const uint32_t &aicNum)
{
    ...
    while (true) {
        ...
        OP_CHECK_IF(lastDimValue % numInOneBlk != 0,
                    OPS_REPORT_VECTOR_INNER_ERR(context->GetNodeName(),
                                                ...
                                                "but now the inner axis size is %ld.",
                                                lastDimValue),
                    return ge::GRAPH_FAILED);
    }
...
ge::graphStatus GMMTiling::CheckMKN(const gert::TilingContext *context)
{
    ...
    OP_CHECK_IF(mmDataTypeSize_ == 0,
                OPS_REPORT_VECTOR_INNER_ERR(context->GetNodeName(), "GMM get mm dtype[%s] size is 0.",
    ...
    OP_CHECK_IF(numInOneBlk == 0, OPS_REPORT_VECTOR_INNER_ERR(context->GetNodeName(), "GMM numInOneBlk cannot be 0."),
                return ge::GRAPH_FAILED);
    int64_t maxMKN = INT_MAX / numInOneBlk * numInOneBlk;
    OP_CHECK_IF(
        maxM_ > maxMKN || maxN_ > maxMKN || maxK_ > maxMKN,
        OPS_REPORT_VECTOR_INNER_ERR(context->GetNodeName(), "32B-aligned m, n or k axis is out of range int32!"),
        return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus GMMTiling::PrepareTilingData(const gert::TilingContext *context)
{
    ...
    OP_CHECK_IF(GMMGetAttrs(context) != ge::GRAPH_SUCCESS,
                OPS_REPORT_VECTOR_INNER_ERR(context->GetNodeName(), "GMMGetAttrs failed"), return ge::GRAPH_FAILED);
    ...
    xKDim_ = transposeX_ ?
                 0U :
                 xDimNum_ - 1U; // 0: when x is transposed, the first dim is k; -1：otherwise, the last dim is k
    ...
    weightNDim_ = transposeWeight_ ?
                      wDimNum - 2U :
                      wDimNum - 1U; // -2: when w is transposed, the last 2 dim is n; -1: otherwise, the last dim is n
    weightKDim_ = transposeWeight_ ?
                      wDimNum - 1U :
                      wDimNum - 2U; // -2: when w is transposed, the last 1 dim is k; -1: otherwise, the last 2 dim is k
    nzFactor_ = 1;                  // init
    if (wFormat_ == matmul_tiling::CubeFormat::NZ) {
        ...
        if (wDimNum >= 4U) { // 4: least dim num of nz format tensor
            weightNDim_ = transposeWeight_ ? wDimNum - 3U :
                                             wDimNum - 4U; // -3: when w is transposed, the last 3 dim is n/nzFactor;
                                                           // -4: when w has nz format, the last 4 dim is n/nzFactor
            // nzFactor_ is a factor used to compute n axis size. If weight is transposed, nzFactor_ is 16; otherwise
            // nzFactor_ is 16 for bf16, 32 for int8
    ...
    isSingleY_ =
        (splitItem_ == 2 ||
         splitItem_ ==
             3); // 2: when x is multi-tensor, y is single-tensor; 3: when x is single-tensor, y is single-tensor
...
ge::graphStatus GMMTiling::SplitKSingleXSeparatedWeight(const gert::TilingContext *context, const gert::Shape &xShape,
                                                        const gert::Shape &wShape)
{
    int64_t m = GMMGetBS(xShape);
    int64_t k = xShape.GetDim(xKDim_);
...
ge::graphStatus GMMTiling::Init(const gert::TilingContext *context)
{
    ...
    OP_CHECK_IF(CheckMKN(context) != ge::GRAPH_SUCCESS,
                OPS_REPORT_VECTOR_INNER_ERR(context->GetNodeName(), "GMM CheckMKN failed."), return ge::GRAPH_FAILED);
    ...
    tilingData.gmmBaseParams.set_k(maxK_);
    tilingData.gmmBaseParams.set_n(maxN_);
    OP_LOGI(context->GetNodeName(), "GMM_tiling: groupNum_ is %u, maxM_ is %ld, maxK_ is %ld, maxN_ is %ld.", groupNum_,
            maxM_, maxK_, maxN_);
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus GMMTiling::GetPerGroupNum(const gert::TilingContext *context)
{
    ...
    if ((isSingleWeight_ && wDimNum > 2UL && dimNum == 3L) ||
        (!isSingleWeight_ && dimNum == 2L)) { // 2 and 3: dim threshold
...
void GMMTiling::DivideUbAndSetWorkspaceAntiquant(size_t *workspaces, const uint32_t &aicNum, uint32_t &ubSize)
{
    if (isA16W8Msd_) {
        ...
        ubSize = static_cast<uint32_t>(ubSize_ - (static_cast<uint64_t>(baseM_) / A16W8_MSD_STEP) * alignedAicNum *
                                                     sizeof(float));
        ...
        for (uint32_t i = 0; i < groupNum_; i++) {
            ...
            int32_t kInList =
                isAllSingleTensor ? kList_[0] : kList_[i]; // in s-s-s case，k only exits in the first of the list
            int32_t nInList =
                isAllSingleTensor ? nList_[0] : nList_[i]; // in s-s-s case，n only exits in the first of the list
        ...
        // when minK * baseN * coreNum * sizeof(float16) > 12M, it goes into antiquantPerformance branch (12M is
        // obtained by test).
        int32_t dimMN = CeilDiv(CeilDiv(maxM_, groupNum_), baseM_) * CeilDiv(maxN_, baseN_);
        bool goodCubeUtility = dimMN * (xDType_ == ge::DT_BF16 ? 2 : 1) >=
                               static_cast<int32_t>(aicNum * 0.4); // 0.4: a factor, in practice.
        antiquantPerformance_ =
            goodCubeUtility &&
            static_cast<int64_t>(minK_) * baseN_ * static_cast<int64_t>(aicNum) >= ANTIQUANT_PERFORMANCE_THRESHOLD &&
            !transposeWeight_;
...
bool GMMTiling::TryFullLoadA(int32_t baseM, const GMMCompileInfo *compileInfoPtr)
{
    ...
    static const std::unordered_map<ge::DataType, float> typeSizeMap = {
        {ge::DT_FLOAT, 4.0f}, {ge::DT_FLOAT16, 2.0f}, {ge::DT_BF16, 2.0f}, {ge::DT_INT8, 1.0f}, {ge::DT_INT4, 0.5f}};
...
ge::graphStatus GMMTiling::IsOutputDisableL2Cache(gert::TilingContext *context, const GMMCompileInfo *compileInfoPtr)
{
    OP_CHECK_IF(compileInfoPtr == nullptr,
                OPS_REPORT_CUBE_INNER_ERR(context->GetNodeName(), "compileInfoPtr is nullptr."),
                return ge::GRAPH_FAILED);
...
ge::graphStatus GMMTiling::DivideUbAndSetWorkspace(gert::TilingContext *context, const uint32_t &aicNum)
{
    ...
    if ((xDType_ == ge::DT_BF16 || xDType_ == ge::DT_FLOAT16)) {
        ...
        OP_CHECK_IF(GetPerGroupNum(context) != ge::GRAPH_SUCCESS,
                    OPS_REPORT_VECTOR_INNER_ERR(context->GetNodeName(), "GetPerGroupNum failed."),
                    return ge::GRAPH_FAILED);
        ...
        if (isFixedAxisMove_) {
            ...
            ubSize = perTokenOrPerGroupSize_ == 1U ? // is perToken
                         static_cast<uint32_t>(ubSize_ - (static_cast<uint64_t>(baseN_) * scaleDataTypeSize +
                                                          static_cast<uint64_t>(baseM_) * sizeof(float)) *
                                                             QUEUE_DOUBLE_BUFFER) :
        ...
        ubSize =
            perTokenOrPerGroupSize_ == 1U ? // is perToken
                static_cast<uint32_t>(ubSize_ - (static_cast<uint32_t>(baseM_) * sizeof(float)) * QUEUE_DOUBLE_BUFFER) :
                ubSize_;
        OP_CHECK_IF(SetWorkspscesPerTokenQuant(aicNum, workspaces) != ge::GRAPH_SUCCESS,
                    OPS_REPORT_VECTOR_INNER_ERR(context->GetNodeName(), "SetWorkspscesPerTokenQuant failed."),
                    return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(GMMSetUbDivideBlk() != ge::GRAPH_SUCCESS,
                OPS_REPORT_VECTOR_INNER_ERR(context->GetNodeName(), "GMMSetUbDivideBlk failed."),
                return ge::GRAPH_FAILED);
    OP_CHECK_IF(GMMCalUbSize(context, ubSize) != ge::GRAPH_SUCCESS,
                OPS_REPORT_VECTOR_INNER_ERR(context->GetNodeName(), "GMMCalUbSize failed."), return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
...
void GMMTiling::FindBestUsedCoreNumOneGroup(const uint32_t aicNum)
{
    ...
    if ((aicNum * 3U > totalCoreNums && totalCoreNums % aicNum <= aicNum / 2U) ||
        totalCoreNums < aicNum) { // 2: half of aicNum
...
ge::graphStatus GMMTiling::SetWorkspscesPerTokenQuant(const uint32_t aicNum, size_t *workspaces)
{
    ...
    if (isA4W4_) {
        if (isA4W4Optimize_) {
            ...
            workspaces[0] +=
                2UL * BASEM_OPTIMIZE * BEST_BASEN * usedCoreNum_ * sizeof(short); // a4w4 mmout dtype is half
        } else {
...
bool GMMTiling::StaticTilingProcess(gert::TilingContext *context)
{
    ...
    if ((xDType_ != ge::DT_INT8 || weightDtype_ != ge::DT_INT8) ||
        (isSingleX_ == 0 || isSingleWeight_ == 0 || isSingleY_ == 0) || hasBias_ || actType_ != 0U ||
        tilingData.gmmBaseParams.get_isPreTiling() != 0 || tilingData.gmmBaseParams.get_groupType() != 0 ||
...
bool GMMTiling::CheckTilingMatchStaticValue()
{
    if (tilingData.mmTilingData.get_depthA1() != STATIC_TILING_DEPTH_A1_B1 ||
        ...
        tilingData.mmTilingData.get_stepM() != 1 || tilingData.mmTilingData.get_stepN() != 1 ||
        ...
        tilingData.mmTilingData.get_dbL0B() != DOUBLE_BUFFER_L0A_L0B || tilingData.mmTilingData.get_dbL0C() != 1 ||
...
void GMMTiling::PrintTilingInfo(gert::TilingContext *context)
{
    ...
    OP_LOGD(context->GetNodeName(), "GMM_tiling_new: isOutputDisableL2Cache is %d.",
            tilingData.gmmBaseParams.get_isOutputDisableL2Cache());
    ...
    if (transposeWeight_) {
        ...
        if ((xDType_ == ge::DT_BF16 || xDType_ == ge::DT_FLOAT16) &&
            ...
            if (perTokenOrPerGroupSize_ > 0U) {
                ...
                ubBaseN = std::min<uint32_t>(BEST_UB_BASEN,
                                             std::max<uint32_t>(MIN_UB_BASEN, (ubCalSize / ubBaseK + MIN_UB_BASEN - 1) /
...
uint32_t GMMTiling::GetTplDataType(const ge::DataType &dtype)
{
    static const std::map<ge::DataType, uint32_t> dtype_map = {
        {ge::DT_INT4, GMM_TPL_INT4}, {ge::DT_INT8, GMM_TPL_INT8},   {ge::DT_FLOAT16, GMM_TPL_FLOAT16},
        {ge::DT_BF16, GMM_TPL_BF16}, {ge::DT_INT32, GMM_TPL_INT32}, {ge::DT_FLOAT, GMM_TPL_FLOAT}};
...
bool GMMTiling::IsAivAicRatioTwoRequired()
{
    ...
    const bool needs_double_vector =
...
{
    bool isCorrectShape =
        (maxK_ == FIXAXISMOVE_K1 && maxN_ == FIXAXISMOVE_N1) || (maxK_ == FIXAXISMOVE_K2 && maxN_ == FIXAXISMOVE_N2);
    bool isGroupCorrect = (groupNum_ == FIXAXISMOVE_GROUP_NUM);
    bool isTuningInRange = (tuningConfig_ >= FIXAXISMOVE_PERM_LOWER) && (tuningConfig_ <= FIXAXISMOVE_PERM_UPPER);
    bool isDataTypeCorrect =
        yDtype_ == ge::DT_FLOAT16 && scaleDtype_ == ge::DT_FLOAT && perTokenScaleDtype_ == ge::DT_FLOAT;
    bool isConfigCorrect = !transposeX_ &&
                           (splitItem_ == FIXAXISMOVE_SPLIT_ITEM2 || splitItem_ == FIXAXISMOVE_SPLIT_ITEM3) &&
                           (groupListType_ == FIXAXISMOVE_GROUP_LIST_TYPE) && (groupType_ == FIXAXISMOVE_GROUP_TYPE) &&
                           (actType_ == 0) && !transposeWeight_;
    bool isWorkspaceValid =
        (static_cast<int64_t>(FixedAxisMoveWorkspace_) <= tuningConfigWorkspace_) || (tuningConfigWorkspace_ == -1);
    bool isFormatValid = (wFormat_ == matmul_tiling::CubeFormat::NZ);

    return isCorrectShape && isTuningInRange && isGroupCorrect && isA8W8_ && isDataTypeCorrect && isConfigCorrect &&
           isWorkspaceValid && !hasBias_ && isFormatValid;
}
...
bool GMMTiling::IsA4W4OptimizeCondition()
{
    ...
    bool isTuningInRange = (tuningConfig_ >= A4W4OPTIMIZE_PERM_LOWER) && (tuningConfig_ <= A4W4OPTIMIZE_PERM_UPPER);
    bool isPerGroupValid = isPerGroup_ && quantGroupSize_ == A4W4OPTIMIZE_QUANT_GROUP_SIZE;
    bool isDataTypeValid =
        yDtype_ == ge::DT_BF16 && scaleDtype_ == ge::DT_UINT64 && perTokenScaleDtype_ == ge::DT_FLOAT;
    bool isConfigValid = !transposeX_ &&
                         (splitItem_ == A4W4OPTIMIZE_SPLIT_ITEM2 || splitItem_ == A4W4OPTIMIZE_SPLIT_ITEM3) &&
                         (groupListType_ == A4W4OPTIMIZE_GROUP_LIST_TYPE) && (groupType_ == A4W4OPTIMIZE_GROUP_TYPE) &&
                         (actType_ == 0) && !transposeWeight_;
    bool isFormatValid = (wFormat_ == matmul_tiling::CubeFormat::NZ);
...
void GMMTiling::GMMSetTplTilingKey(gert::TilingContext *context)
{
    ...
    } else if (!transposeX_ && xDType_ == weightDtype_ &&
               (xDType_ == ge::DT_FLOAT16 || xDType_ == ge::DT_BF16 || xDType_ == ge::DT_FLOAT)) {
        aivAicRatio = static_cast<uint32_t>(GROUPED_MATMUL_CUBE_ONLY);
    }
    ...
    if (a8w4KernelTemplate == GROUPED_MATMUL_A8W4_KERNEL_TEMPLATE_NONE &&
        ...
        aivAicRatio != GROUPED_MATMUL_AIV_AIC_RATIO_2 && !isFixedAxisMove_ && StaticTilingProcess(context)) {
        isStaticTilingApi = 1U;
    }

    const uint64_t tilingKey =
        GET_TPL_TILING_KEY(GetTplDataType(xDType_), GetTplDataType(weightDtype_), GetTplDataType(yDtype_),
                           transposeX_ ? 1UL : 0UL, transposeWeight_ ? 1UL : 0UL, static_cast<uint64_t>(groupListType_),
                           isStaticTilingApi, a8w4KernelTemplate, a16w8KernelTemplate, aivAicRatio, isEnableFixedAxis);
...
ge::graphStatus GMMTiling::GMMGetAttrs(const gert::TilingContext *context)
{
    ...
    isA16W16_ = (xDType_ == ge::DT_FLOAT16 && weightDtype_ == ge::DT_FLOAT16) ||
                (xDType_ == ge::DT_BF16 && weightDtype_ == ge::DT_BF16);
    auto compileInfoPtr = context->GetCompileInfo<GMMCompileInfo>();
    OP_CHECK_NULL_WITH_CONTEXT(context, compileInfoPtr); // check compileInfoPtr is not null
    if (groupListType_ == GROUP_LIST_SPARSE_M) {
    ...
    tuningConfig_ =
        (tuningConfigPtr != nullptr && tuningConfigPtr->GetSize() > TUNING_CONFIG_TOKEN_PER_EXPECT_INDEX) ?
            (reinterpret_cast<const int64_t *>(tuningConfigPtr->GetData()))[TUNING_CONFIG_TOKEN_PER_EXPECT_INDEX] :
            0;
    tuningConfigWorkspace_ =
        (tuningConfigPtr != nullptr && tuningConfigPtr->GetSize() > TUNING_CONFIG_ALLOW_WORKSPACE_INDEX) ?
            (reinterpret_cast<const int64_t *>(tuningConfigPtr->GetData()))[TUNING_CONFIG_ALLOW_WORKSPACE_INDEX] :
            0;
...
void GMMTiling::FullLoadK(const GMMCompileInfo *compileInfoPtr)
{
    if (wFormat_ == matmul_tiling::CubeFormat::ND && !transposeWeight_ && !transposeX_ && xDType_ == weightDtype_ &&
        (weightDtype_ == ge::DT_FLOAT16 || weightDtype_ == ge::DT_BF16) && maxM_ >= FULL_K_M_E_THRESHOLD * groupNum_ &&
        maxN_ == FULL_K_N_THRESHOLD && maxK_ <= FULL_K_MAX_K_THRESHOLD && maxK_ >= FULL_K_MIN_K_THRESHOLD) {
                                        ...
                                        int64_t mInMM, uint32_t &mmStepKa, uint32_t &mmStepKb)
{
    ...
    if (isA4W4_) {
        // 2: double buffer
        mmStepKa = static_cast<uint32_t>(
            (l1ASize / 2UL) /
            static_cast<uint64_t>(INT4_DATA_TYPE_SIZE * static_cast<float>(baseM_) * static_cast<float>(baseK_)));
        // 2: double buffer
        mmStepKb = static_cast<uint32_t>(
            (l1BSize / 2UL) /
            static_cast<uint64_t>(INT4_DATA_TYPE_SIZE * static_cast<float>(baseN_) * static_cast<float>(baseK_)));
    } else {
        // 2: double buffer
    ...
    if (compileInfoPtr->socVersion == platform_ascendc::SocVersion::ASCEND310P &&
        ...
        mmStepKa = std::min<uint32_t>(
            ...
            std::max(
                1,
                128 / baseK_)); // 128: nz inner block size. In practice, baseK_*mmStepKa=128 makes performance better.
    }
...
ge::graphStatus GMMTiling::GMMSetMMTiling(const gert::TilingContext *context, const GMMCompileInfo *compileInfoPtr)
{
    ...
    int64_t mInMM =
        isA16W8Msd_ ? static_cast<int64_t>(A16W8_MSD_STEP) * maxM_ : maxM_; // if msd, m in matmul should mul steps
...
ge::graphStatus GMMTiling::CalMMTiling(const gert::TilingContext *context, const GMMCompileInfo *compileInfoPtr)
{
    ...
    bool tuningConfigFlag =
        (tuningConfig_ > tuningConfigBaseLowerLimit && tuningConfig_ <= tuningConfigBaseUpperLimit) ||
        (yDtype_ == ge::DT_INT32 && tuningConfig_ > 0 && tuningConfig_ <= tuningConfigBaseLowerLimit &&
         (maxK_ == maxKLimit || maxK_ == altKLimit) && maxN_ == maxNLimit);
    ...
    if (isA16W8Msd_ && maxN_ >= 2048 && !transposeWeight_) {
        ...
        baseN_ =
            tuningConfig_ > 64 ? BEST_BASEN : BEST_BASEN_A4W4; // 64 : when token in each group > 64, set baseN to 256
    } else {
        baseN_ = BEST_BASEN;
    }
    if (groupType_ == SPLIT_K && isSingleX_ && isSingleWeight_ && isA16W16_ &&
    ...
    OP_CHECK_IF(baseK_ == 0, OPS_REPORT_VECTOR_INNER_ERR(context->GetNodeName(), "baseK_ cannot be 0."),
                return ge::GRAPH_FAILED);
    // according to the double buffer enabled L0A/L0C, compute baseM(cube)
    uint32_t maxBaseM =
        static_cast<uint32_t>(compileInfoPtr->l0CSize / (static_cast<uint32_t>(baseN_) * FP32_DATATYPE_SIZE));
    baseM_ = isA4W4_ ? std::min<uint32_t>((compileInfoPtr->l0ASize / DOUBLE_BUFFER_L0A_L0B) /
                                              static_cast<uint32_t>(baseK_ * INT4_DATA_TYPE_SIZE),
                                          maxBaseM) :
                       std::min<uint32_t>((compileInfoPtr->l0ASize / DOUBLE_BUFFER_L0A_L0B) /
                                              (static_cast<uint32_t>(baseK_) * mmDataTypeSize_),
                                          maxBaseM);

    if (!isA16W8Msd_) {
        baseM_ = baseM_ > maxM_ ? SixteenAlign(maxM_, true) : SixteenAlign(static_cast<uint32_t>(baseM_));
    } else {
        baseM_ = baseM_ > A16W8_MSD_STEP * maxM_ ?
                     static_cast<int32_t>(SixteenAlign(static_cast<int64_t>(A16W8_MSD_STEP) * maxM_, true)) :
                     SixteenAlign(static_cast<int64_t>(baseM_));
    }
    if (baseM_ > MAX_BASEM) {
        baseM_ = MAX_BASEM;
    }
    OP_CHECK_IF(baseM_ == 0, OPS_REPORT_VECTOR_INNER_ERR(context->GetNodeName(), "baseM_ cannot be 0."),
                return ge::GRAPH_FAILED);
...
static void PrintA8W4HPTiling(gert::TilingContext *context, A8W4HPTiling *data)
{
    ...
    OP_LOGD(context->GetNodeName(), "  splitRecord=[%u,%u,%u]", data->get_splitRecord()[0], data->get_splitRecord()[1],
            data->get_splitRecord()[TWO]);
    OP_LOGD(context->GetNodeName(), "  workspaceOffset=%llu",
            static_cast<long long unsigned int>(data->get_workspaceOffset()));
}
...
ge::graphStatus GMMTiling::A16W4MsdTiling(gert::TilingContext *context, const GMMCompileInfo *compileInfoPtr)
{
    ...
    context->SetTilingKey(
        GET_TPL_TILING_KEY(GMM_TPL_BF16, GMM_TPL_INT4, GMM_TPL_BF16, 0, 1, static_cast<uint64_t>(groupListType_), 0,
...
ge::graphStatus GMMTiling::A8W4Tiling(gert::TilingContext *context, const GMMCompileInfo *compileInfoPtr)
{
    ...
    bool useHighPerf =
        (tuningConfigPtr != nullptr && tuningConfigPtr->GetSize() > TUNING_CONFIG_A8W4_SPEC_SCENARIO_INDEX) ?
            ((reinterpret_cast<const int64_t *>(tuningConfigPtr->GetData()))[TUNING_CONFIG_A8W4_SPEC_SCENARIO_INDEX] ==
             1) :
            false;
    if (useHighPerf) {
        if (!isSingleTensor) {
        ...
        if (quantGroupNum > 1) {
            OP_LOGW(context->GetNodeName(),
                    "W4A8 autotiling: The quantGroupNum is %u, which is invalid. "
                    "The expected value is 1 (only support perchannel)",
                    quantGroupNum);
        }
        const auto transposeWeightPtr = attr->GetAttrPointer<bool>(ATTR_INDEX_TRANS_W);
        ...
        if (offset != nullptr) {
            ...
            if (isSingleTensor == 1) {
                ...
                if (offsetDimNum == OFFSET_DIM_A8W4 && offsetDim0 == groupNum && offsetDim1 == 1 && offsetDim2 == n) {
                    ...
                    OP_LOGW(context->GetNodeName(),
                            "GMM A8W4: offset's shape is invalid, "
                            "If you want to enable offset, the expected shape is (%u,1,%u), and the current shape is "
                            "(%ld,%ld,%ld).",
        ...
        const int is_in_a8w4_white_list =
            ((tuningConfig != 0 && tuningConfig <= 64) || (A8W4_PRETILING_WHITE_LIST.count(mKNList))) &&
            quantGroupNum != 0 && k / quantGroupNum == 256 && k % quantGroupNum == 0 && withOffset == 0 &&
            k % 64 == 0 && n % 64 == 0; // 256: 新方案只支持256 pergroup
        uint32_t calc_m = 1U;
        if (groupNum != 0U) {
            calc_m = m / groupNum;
        }
        ...
        if (!isMSD) {
            ...
            mm.SetAType(matmul_tiling::TPosition::GM, matmul_tiling::CubeFormat::ND, matmul_tiling::DataType::DT_INT8,
                        false);
            if (wNZ) {
            ...
            context->SetTilingKey(
                GET_TPL_TILING_KEY(GMM_TPL_INT8, GMM_TPL_INT4, yDtype, 0, 0, GROUPED_MATMUL_GROUP_LIST_TYPE_COUNT, 0,
                                   ...
                                   GROUPED_MATMUL_A16W8_KERNEL_TEMPLATE_NONE, GROUPED_MATMUL_AIV_AIC_RATIO_2, 0));
            tilingDataA8W4.SaveToBuffer(context->GetRawTilingData()->GetData(),
            ...
            // 增加动态Tiling部分
            uint32_t A8W4_MSD_SINGLE_N =
                FindBestSingleNA8W4(A8W4_MSD_BASE_M_NEW, A8W4_MSD_BASE_N_NEW, avg_m, n, groupNum, aicNum);
            mm.SetAType(matmul_tiling::TPosition::GM, matmul_tiling::CubeFormat::ND, matmul_tiling::DataType::DT_INT4,
                        false);
            if (wNZ) {
                mm.SetBType(matmul_tiling::TPosition::GM, matmul_tiling::CubeFormat::NZ,
                            matmul_tiling::DataType::DT_INT4, false);
            } else {
                mm.SetBType(matmul_tiling::TPosition::GM, matmul_tiling::CubeFormat::ND,
                            matmul_tiling::DataType::DT_INT4, false);
            }
            ...
            if (is_in_a8w4_white_list) {
                ...
                OP_LOGI(context->GetNodeName(),
                        "GMM A8W4 tiling: baseM is %u, baseN is %u, baseK is %u, tuningConfig is %ld.",
                        A8W4_MSD_BASE_M_NEW, A8W4_MSD_BASE_N_NEW, A8W4_MSD_BASE_K_NEW, tuningConfig);
            } else {
                ...
                OP_LOGI(context->GetNodeName(),
                        "GMM A8W4 tiling: baseM is %u, baseN is %u, baseK is %u, tuningConfig is %ld.", A8W4_MSD_BASE_M,
                        A8W4_MSD_BASE_N, A8W4_MSD_BASE_K, tuningConfig);
            }
            if (mm.GetTiling(tilingDataA8W4.mmTilingData) == -1) {
                return ge::GRAPH_FAILED;
            }
            ...
            OP_LOGI(context->GetNodeName(), "GMM_tiling: baseM is %u, baseK is %u, baseN is %u.", A8W4_MSD_BASE_M,
                    A8W4_MSD_BASE_K, A8W4_MSD_BASE_N);
            context->SetScheduleMode(1); // set as batchmod for template using SyncAll
            ...
            if (enableCV11) {
                context->SetTilingKey(GET_TPL_TILING_KEY(
                    GMM_TPL_INT8, GMM_TPL_INT4, yDtype, 0, 0, GROUPED_MATMUL_GROUP_LIST_TYPE_COUNT, 0,
                    a8w4KernelTemplate, GROUPED_MATMUL_A16W8_KERNEL_TEMPLATE_NONE, GROUPED_MATMUL_AIV_AIC_RATIO_1, 0));
            } else {
                context->SetTilingKey(GET_TPL_TILING_KEY(
                    GMM_TPL_INT8, GMM_TPL_INT4, yDtype, 0, 0, GROUPED_MATMUL_GROUP_LIST_TYPE_COUNT, 0,
                    a8w4KernelTemplate, GROUPED_MATMUL_A16W8_KERNEL_TEMPLATE_NONE, GROUPED_MATMUL_AIV_AIC_RATIO_2, 0));
            }
            size_t *workspaces = context->GetWorkspaceSizes(1); // get second variable
            OP_CHECK_NULL_WITH_CONTEXT(context, workspaces);    // check workspaces is not null
            workspaces[0] = SYS_WORKSPACE_SIZE;                 // default size
            if (is_in_a8w4_white_list) {
                workspaces[0] += static_cast<size_t>(
                    (cvParallNum * aicNum * A8W4_MSD_BASE_M * A8W4_MSD_BASE_N * static_cast<uint32_t>(sizeof(short))) *
                    TWO);
            } else {
                workspaces[0] += static_cast<size_t>(
                    (cvParallNum * aicNum * singleN * singleM * static_cast<uint32_t>(sizeof(int32_t)) * EIGHT));
                ...
                tuningConfigWorkspace_ =
                    (tuningConfigPtr != nullptr && tuningConfigPtr->GetSize() > TUNING_CONFIG_ALLOW_WORKSPACE_INDEX) ?
                        (reinterpret_cast<const int64_t *>(
                            tuningConfigPtr->GetData()))[TUNING_CONFIG_ALLOW_WORKSPACE_INDEX] :
                        0;
...
ASCENDC_EXTERN_C ge::graphStatus TilingGMM(gert::TilingContext *context)
{
    ...
    OP_CHECK_IF(tiling.Init(context) != ge::GRAPH_SUCCESS,
                OPS_REPORT_VECTOR_INNER_ERR(context->GetNodeName(), "GMM tiling init failed"), return ge::GRAPH_FAILED);
...
ASCENDC_EXTERN_C ge::graphStatus TilingPrepareForGMM(gert::TilingParseContext *context)
{
    ...
    OP_CHECK_IF((compileInfoPtr->aicNum == 0 || compileInfoPtr->ubSize == 0 || compileInfoPtr->l1Size == 0 ||
