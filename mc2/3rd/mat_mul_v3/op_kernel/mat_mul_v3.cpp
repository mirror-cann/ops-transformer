...
#define MMV3_IMPL_CLASS_TRANS(tilingData, tilingGM, transA, transB, templateClass, ...)                                \
    do {                                                                                                               \
        uint64_t tilingdata_offset =                                                                                   \
            (sizeof(Mc2MatMulV3TilingData) > TILINGDATA_OFFSET) ?                                                      \
                0 :                                                                                                    \
                                      ...
                                      GM_ADDR workspaceGM, GM_ADDR tilingGM)
{
    ...
    if (LOADMODE == MAT_MUL_V3_BASE_FULLLOAD && SPLITCOREMODE == MAT_MUL_V3_BASE_SPLIT_K &&
        ...
        MMV3_IMPL_CLASS(Mc2MatmulBaseUnAlignedKernel, format_x1, Mc2MatmulBaseBlock, MM_CFG_VEC_ND2NZ);
    } else if (LOADMODE == MAT_MUL_V3_BASE_FULLLOAD && SPLITCOREMODE == MAT_MUL_V3_BASE_SPLIT_K &&
               FIXOPTI == MAT_MUL_V3_BASE_FIXOPTI && MIXND2NZ == MAT_MUL_V3_MIXND2NZ_FALSE) {
        MMV3_IMPL_CLASS(Mc2MatmulBaseKernel, format_x1, Mc2MatmulBaseBlock, MM_CFG_VEC_ND2NZ);
    }
#elif defined(FORMAT_X2) && FORMAT_X2 == FORMAT_FRACTAL_NZ
    if (LOADMODE == MAT_MUL_V3_BASE_FULLLOAD && SPLITCOREMODE == MAT_MUL_V3_BASE_SPLIT_K &&
        FIXOPTI == MAT_MUL_V3_BASE_FIXOPTI && MIXND2NZ == MAT_MUL_V3_MIXND2NZ_TRUE) {
        MMV3_IMPL_CLASS(Mc2MatmulBaseUnAlignedKernel, format_x1, Mc2MatmulBaseBlock, MM_CFG_NO_PRELOAD);
    } else if (LOADMODE == MAT_MUL_V3_BASE_FULLLOAD && SPLITCOREMODE == MAT_MUL_V3_BASE_SPLIT_K &&
               FIXOPTI == MAT_MUL_V3_BASE_FIXOPTI && MIXND2NZ == MAT_MUL_V3_MIXND2NZ_FALSE) {
        MMV3_IMPL_CLASS(Mc2MatmulBaseKernel, format_x1, Mc2MatmulBaseBlock, MM_CFG_NO_PRELOAD);
    }
#else
    if (LOADMODE == MAT_MUL_V3_BASE_FULLLOAD && SPLITCOREMODE == MAT_MUL_V3_BASE_SPLIT_K &&
        FIXOPTI == MAT_MUL_V3_BASE_FIXOPTI && MIXND2NZ == MAT_MUL_V3_MIXND2NZ_TRUE) {
        MMV3_IMPL_CLASS(Mc2MatmulBaseUnAlignedKernel, format_x1, Mc2MatmulBaseBlock, MM_CFG_NO_PRELOAD);
    } else if (LOADMODE == MAT_MUL_V3_BASE_FULLLOAD && SPLITCOREMODE == MAT_MUL_V3_SINGLE_CORE_SPLIT_K &&
               FIXOPTI == MAT_MUL_V3_BASE_FIXOPTI && MIXND2NZ == MAT_MUL_V3_MIXND2NZ_FALSE) {
        MMV3_IMPL_CLASS(Mc2MatMulSingleCoreSplitKKernel, format_x1, Mc2MatmulSingleCoreSplitKBaseBlock,
                        MM_CFG_PRELOAD_MK);
    } else if (LOADMODE == MAT_MUL_V3_BASE_FULLLOAD && SPLITCOREMODE == MAT_MUL_V3_SINGLE_CORE_NKM_SPLIT_K &&
               FIXOPTI == MAT_MUL_V3_BASE_FIXOPTI && MIXND2NZ == MAT_MUL_V3_MIXND2NZ_FALSE) {
        MMV3_IMPL_CLASS(Mc2MatMulSingleCoreSplitKKernel, format_x1, Mc2MatmulSingleCoreSplitKBaseBlock,
                        MM_CFG_PRELOAD_NK, true);
    } else if (LOADMODE == MAT_MUL_V3_BASE_FULLLOAD && SPLITCOREMODE == MAT_MUL_V3_SINGLE_CORE_SPLIT_K_GM_TO_L1 &&
               FIXOPTI == MAT_MUL_V3_BASE_FIXOPTI && MIXND2NZ == MAT_MUL_V3_MIXND2NZ_FALSE) {
        MMV3_IMPL_CLASS(MatMulSingleCoreSplitKKernelGmToL1, format_x1, Mc2MatmulSingleCoreSplitKBaseBlock,
                        MM_CFG_PRELOAD_MK);
    } else if (LOADMODE == MAT_MUL_V3_BASE_FULLLOAD && SPLITCOREMODE == MAT_MUL_V3_SINGLE_CORE_SPLIT_K &&
               FIXOPTI == MAT_MUL_V3_BASE_FIXOPTI && MIXND2NZ == MAT_MUL_V3_MIXND2NZ_TRUE) {
        MMV3_IMPL_CLASS(Mc2MatMulUnAlignedSingleCoreSplitKKernel, format_x1, Mc2MatmulSingleCoreSplitKBaseBlock,
                        MM_CFG_PRELOAD_MK);
    } else if (LOADMODE == MAT_MUL_V3_BASE_FULLLOAD && SPLITCOREMODE == MAT_MUL_V3_SINGLE_CORE_SPLIT_K_GM_TO_L1 &&
               FIXOPTI == MAT_MUL_V3_BASE_FIXOPTI && MIXND2NZ == MAT_MUL_V3_MIXND2NZ_TRUE) {
        MMV3_IMPL_CLASS(Mc2MatMulUnAlignedSingleCoreSplitKKernelGmToL1, format_x1, Mc2MatmulSingleCoreSplitKBaseBlock,
                        MM_CFG_PRELOAD_MK);
    } else if (LOADMODE == MAT_MUL_V3_BASE_FULLLOAD && SPLITCOREMODE == MAT_MUL_V3_DETERMINISTIC_SPLIT_K &&
               FIXOPTI == MAT_MUL_V3_BASE_FIXOPTI && MIXND2NZ == MAT_MUL_V3_MIXND2NZ_FALSE) {
        MMV3_IMPL(Mc2MatMulKernelDeterministicSplitK, format_x1, FIXPIPE_OPT_SELECT::BASE);
    } else if (LOADMODE == MAT_MUL_V3_BASE_FULLLOAD && SPLITCOREMODE == MAT_MUL_V3_DETERMINISTIC_SPLIT_K &&
               FIXOPTI == MAT_MUL_V3_BASE_FIXOPTI && MIXND2NZ == MAT_MUL_V3_MIXND2NZ_TRUE) {
        MMV3_IMPL(Mc2MatMulUnAlignedKernelDeterministicSplitK, format_x1, FIXPIPE_OPT_SELECT::BASE);
    } else if (LOADMODE == MAT_MUL_V3_BASE_FULLLOAD && SPLITCOREMODE == MAT_MUL_V3_MULTI_CORE_SPLIT_K &&
               FIXOPTI == MAT_MUL_V3_BASE_FIXOPTI && MIXND2NZ == MAT_MUL_V3_MIXND2NZ_FALSE) {
        MMV3_IMPL(Mc2MatMulMultiCoreSplitK, format_x1, FIXPIPE_OPT_SELECT::BASE);
    } else if (LOADMODE == MAT_MUL_V3_BASE_FULLLOAD && SPLITCOREMODE == MAT_MUL_V3_BASE_SPLIT_K &&
               FIXOPTI == MAT_MUL_V3_BASE_FIXOPTI && MIXND2NZ == MAT_MUL_V3_MIXND2NZ_FALSE) {
        MMV3_IMPL_CLASS(Mc2MatmulBaseKernel, format_x1, Mc2MatmulBaseBlock, MM_CFG_NO_PRELOAD);
    } else if (LOADMODE == MAT_MUL_V3_AL1_FULLLOAD && SPLITCOREMODE == MAT_MUL_V3_BASE_SPLIT_K &&
               FIXOPTI == MAT_MUL_V3_BASE_FIXOPTI && MIXND2NZ == MAT_MUL_V3_MIXND2NZ_FALSE) {
        MMV3_IMPL_CLASS(Mc2MatmulBaseKernelAL1FullLoad, format_x1, Mc2MatmulBaseBlock, MM_CFG_MDL);
    } else if (LOADMODE == MAT_MUL_V3_BL1_FULLLOAD && SPLITCOREMODE == MAT_MUL_V3_BASE_SPLIT_K &&
               FIXOPTI == MAT_MUL_V3_BASE_FIXOPTI && MIXND2NZ == MAT_MUL_V3_MIXND2NZ_FALSE) {
        MMV3_IMPL_CLASS(MatmulBaseKernelBL1FullLoad, format_x1, Mc2MatmulBaseBlock, MM_CFG_NO_PRELOAD);
    } else if (LOADMODE == MAT_MUL_V3_BL1_FULLLOAD && SPLITCOREMODE == MAT_MUL_V3_BASE_SPLIT_K &&
               FIXOPTI == MAT_MUL_V3_BASE_ENABLE_ALIGNOUT && MIXND2NZ == MAT_MUL_V3_MIXND2NZ_FALSE) {
        MMV3_IMPL_CLASS(Mc2MatmulBaseUnalignedNKernel, format_x1, Mc2MatmulBaseBlock, MM_CFG_NO_PRELOAD,
                        MatmulCallBackFunc<nullptr, nullptr, CopyBL1>);
    } else if (LOADMODE == MAT_MUL_V3_BL1_FULLLOAD && SPLITCOREMODE == MAT_MUL_V3_BASE_SPLIT_K &&
               FIXOPTI == MAT_MUL_V3_BASE_FIXOPTI && MIXND2NZ == MAT_MUL_V3_MIXND2NZ_TRUE) {
        MMV3_IMPL_CLASS(Mc2MatmulBaseUnAlignedKernelBL1FullLoad, format_x1, Mc2MatmulBaseBlock, MM_CFG_NO_PRELOAD,
                        MatmulCallBackFunc<nullptr, nullptr, CopyBL1>);
    } else if (LOADMODE == MAT_MUL_V3_BL1_FULLLOAD && SPLITCOREMODE == MAT_MUL_V3_BASE_SPLIT_K &&
        ...
        MMV3_IMPL_CLASS(Mc2MatmulBaseAToNZWithBL1FixpipeKernel, CubeFormat::NZ, Mc2MatmulBaseBlock, MM_CFG_NO_PRELOAD,
                        MatmulCallBackFunc<nullptr, nullptr, CopyBL1>);
    } else if (LOADMODE == MAT_MUL_V3_BL1_FULLLOAD && SPLITCOREMODE == MAT_MUL_V3_BASE_SPLIT_K &&
               FIXOPTI == MAT_MUL_V3_VEC_NZ2ND_UNALIGNOUT && MIXND2NZ == MAT_MUL_V3_MIXND2NZ_FALSE) {
        MMV3_IMPL_C_CLASS(Mc2MatmulBaseUnalignedNKernel, format_x1, CubeFormat::NZ, Mc2MatmulBaseBlock,
                          MM_CFG_NO_PRELOAD, MatmulCallBackFunc<nullptr, nullptr, CopyBL1>,
                          FIXPIPE_OPT_SELECT::VEC_NZ2ND_UNALIGNOUT);
    } else if (LOADMODE == MAT_MUL_V3_BASE_FULLLOAD && SPLITCOREMODE == MAT_MUL_V3_DETERMINISTIC_SPLIT_K &&
               FIXOPTI == MAT_MUL_V3_VEC_NZ2ND_UNALIGNOUT && MIXND2NZ == MAT_MUL_V3_MIXND2NZ_FALSE) {
        MMV3_IMPL(Mc2MatMulKernelDeterministicSplitK, CubeFormat::NZ, FIXPIPE_OPT_SELECT::VEC_NZ2ND_UNALIGNOUT);
    } else if (LOADMODE == MAT_MUL_V3_BASE_FULLLOAD && SPLITCOREMODE == MAT_MUL_V3_DETERMINISTIC_SPLIT_K &&
               FIXOPTI == MAT_MUL_V3_VEC_NZ2ND_UNALIGNOUT && MIXND2NZ == MAT_MUL_V3_MIXND2NZ_TRUE) {
        MMV3_IMPL(Mc2MatMulUnAlignedKernelDeterministicSplitK, CubeFormat::NZ,
                  FIXPIPE_OPT_SELECT::VEC_NZ2ND_UNALIGNOUT);
    }
#endif
}
