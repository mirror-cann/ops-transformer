...
/*!
 * \file block_epilogue_pertoken_swiglu.hpp
 * \brief
 */
...
public:
    ...
    void operator()(AscendC::GlobalTensor<ElementC> const &gmC, MatrixCoord const &shapeC,
                    ...
                    Arch::Resource<ArchTag> const &resource, uint32_t epilogueCoreNum = 40, float swigluLimit = 0.0f,
    ...
    void operator()(AscendC::GlobalTensor<ElementC> const &gmC, MatrixCoord const &shapeC,
                    ...
                    uint32_t epilogueCoreNum = 40, float swigluLimit = 0.0f, uint32_t gmmOutPreRowStride = 1,
                    Callback &&callback = Callback{})
    {
        ...
        for (uint32_t loopIdx = loopStartIdx; loopIdx < loopStartIdx + tasksForIdx; ++loopIdx) {
            ...
            auto gmTileD = gmD[loopIdx * ChunkTileLen];
            // 拷贝的为一行，所以实际上 K轴给的是blockN还是gmmOutPreRowStride都一样
            LayoutC layoutUbC{1, blockN};
            LayoutC layoutGmC{1, blockN, gmmOutPreRowStride};
